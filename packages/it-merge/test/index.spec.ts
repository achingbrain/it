...
describe('it-merge', () => {
  ...
  it('should support backpressure', async () => {
    ...
    expect(values1Yielded).to.equal(1)
    expect(values2Yielded).to.equal(1)

    const res2 = await gen.next()
    expect(res2).to.have.property('done', false)
    expect(res2).to.have.property('value', 4)
    expect(values1Yielded).to.equal(1, 'first generator continued yielding')
    expect(values2Yielded).to.equal(1, 'second generator continued yielding')

    const res3 = await gen.next()
    expect(res3).to.have.property('done', false)
    expect(res3).to.have.property('value', 1)
    expect(values1Yielded).to.equal(2, 'first generator continued yielding')
    expect(values2Yielded).to.equal(2, 'second generator continued yielding')
