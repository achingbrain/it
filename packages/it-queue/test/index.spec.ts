...
describe('queue', () => {
  ...
  it('should emit empty event when empty', async () => {
    ...
    const { resolve: resolveJob1, promise: job1Promise } = Promise.withResolvers<void>()
    const { resolve: resolveJob2, promise: job2Promise } = Promise.withResolvers<void>()
  ...
  it('should emit success / failure events', async () => {
    const queue = new Queue({ concurrency: 1 })
  ...
  it('should abort jobs', async () => {
    const abortSignalFired = Promise.withResolvers<void>()
    const jobWasRejected = Promise.withResolvers<Error>()
    const queue = new Queue({ concurrency: 1 })
  ...
  it('can be used as a generator', async () => {
    ...
    await expect(all(queue)).to.eventually.deep.equal(results)
  })
  ...
  it('should remove a job from the queue if it is aborted before starting', async () => {
    const queue = new Queue<string>({
      concurrency: 1
    })

    const controller = new AbortController()
    const addEventListenerSpy = Sinon.spy(controller.signal, 'addEventListener')
    const removeEventListenerSpy = Sinon.spy(controller.signal, 'removeEventListener')
    ...
    expect(queue.running).to.equal(1)
    expect(queue.queued).to.equal(2)
